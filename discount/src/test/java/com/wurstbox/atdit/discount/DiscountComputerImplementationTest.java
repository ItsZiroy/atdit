...
public class DiscountComputerImplementationTest {
  ...
  /**
   This method tests whether a customer who does not receive any discount gets
   no discount.<br/> Customer 0 is granted no discount.<br/> Tests: {@link
   DiscountComputerImplementation#computeDiscount(double, int)}<br/> Input: base
   = 100, customer = 0<br/> Expected Output: List with one entry (Text |
   percentage | amount): <ol> <li>"Aggregate", 0, 0</li>
   </ol>
   */
  @Test
  public void noDiscount() {
    ...
    Mockito.when(serviceMock.getDiscountData(0)).thenReturn(new ArrayList<>());
  ...
  /**
   This method tests whether a single-discount customer gets the expected
   discount.<br/> Customer 1 is granted one single discount of 19%.<br/> Tests:
   {@link DiscountComputerImplementation#computeDiscount(double, int)}<br/>
  ...
  public void singleDiscount() {
    ...
    DiscountDataService serviceMock = Mockito.mock(DiscountDataService.class);
    Mockito.when(serviceMock.getDiscountData(1)).thenReturn(mockDBQueryResult);
    var cut = new DiscountComputerImplementation(serviceMock);
  ...
  /**
   This method tests whether a multi-discount customer gets the expected
   discount.<br/> Customer 2 is granted three discounts of 3, 5, and 19%, that
   is 27% altogether.<br/> Tests: {@link
   DiscountComputerImplementation#computeDiscount(double, int)}<br/> Input: base
   = 100, customer = 2<br/> Expected Output: List with four entries (Text |
   percentage | amount): <ol> <li>"Aggregate", 19, 19</li> <li>"Semesterstart",
   5, 5</li> <li>"Aktionswochen", 3, 3</li> <li>"geschenkte Mehrwertsteuer", 19,
   19</li>
   </ol>
   */
  @Test
  public void multipleDiscounts() {
    ...
    DiscountDataService serviceMock = Mockito.mock(DiscountDataService.class);
    Mockito.when(serviceMock.getDiscountData(2)).thenReturn(mockDBQueryResult);
    var cut = new DiscountComputerImplementation(serviceMock);
