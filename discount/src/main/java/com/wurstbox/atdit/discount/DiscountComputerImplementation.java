...
public class DiscountComputerImplementation implements DiscountComputer {
  ...
  /**
   @implSpec To compute the discount, the customer number is looked up in
   database. Associated discounts are retrieved and computed according to the
   rules defined in {@link DiscountComputer}. A database must be available and
   running. Connection details must be provided in "db.properties". Required
   properties are: <ul> <li>url: database connection string except for user and
   passwort</li> <li>user: db user name</li> <li>password: db password</li>
   </ul>
   */
  @Override
  ...
  private void logAggregatedDiscount(List<Discount> result) {
    ...
    log.info("customer receives {}% discount or {}€",
             result.get(0).percentage(), result.get(0).amount());
  }
  ...
  private void logMethodCall(double base, int customer) {
    ...
    log.info("compute discount for customer {} and base price {}", customer,
             base);
  }
  ...
  private void addAggregateDiscounts(List<Discount> result) {
    ...
    for (Discount d : result) {
      aggregate = new Discount(aggregate.description(),
  ...
  private void convertDBDataToResult(double base, List<Discount> result,
                                     List<DiscountDB> dbData) {
    for (DiscountDB discountDB : dbData) {
      double discountValue = base * discountDB.discount() / 100;
      Discount discount = new Discount(discountDB.discountText(),
                                       discountDB.discount(), discountValue);
      result.add(discount);
    }
  }
}
